...
const SearchExercises = ({setExercises, bodyPart, setBodyPart}) => {
    ...
    const handleSearch = async()=>{
        if(search){
            ...
            const searchedExercises = exercisesData.filter((item) => {
                ...
                const bodyPart = item.bodyPart ? item.bodyPart.toLowerCase() : '';
  ...
  return (
    ...
    <Box sx={{position: 'relative', width: '100%', p: '20px'}}>
        <HorizantalScrollbar data={bodyParts} bodyParts bodyPart={bodyPart} setBodyPart = {setBodyPart} isbodyParts />
        </Box>
    </Stack>
    );
};
